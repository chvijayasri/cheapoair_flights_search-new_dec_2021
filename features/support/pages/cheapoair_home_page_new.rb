class CheapoairHomePageNew
  ...
  sleep 5
  # exp_errors = ["Please enter a From city or airport.","Please enter a To city or airport."]
  # actual_error = @browser.lis(class: 'alerts__item').map(&:text)
  # fail ""  if actual_error |= exp_errors
  ...
  def search_dep_airport dep_city, dep_airport
    ...
    sleep 2
    # (line 15 and 16 are same only for the edit box and when using the self we need not include element)
